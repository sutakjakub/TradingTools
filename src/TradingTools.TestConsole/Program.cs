...
namespace TradingTools.TestConsole
{
    class Program
    {
        private static DbContextOptionsBuilder<TradingToolsDbContext> optionsBuilder;
        private static BinanceClientOptions binanceOptions;
        ...
        static async Task Main(string[] args)
        {
            ...
            optionsBuilder = new DbContextOptionsBuilder<TradingToolsDbContext>();
            optionsBuilder.UseSqlServer(config.GetConnectionString("DefaultConnection"));

            binanceOptions = new BinanceClientOptions
            {
            ...
            //    .Include(i => i.T2SymbolInfo)
            //    .Where(p => p.T2SymbolInfo.BaseAsset == "CELR")
            //    .ToList();
            ...
            //using (var writer = new StreamWriter("crypto_gains2.csv"))
            ...
            //{
            //    var binanceSymbols = await exchange.GetAllSymbolsAsync();
            //    var dbSymbols2 = db.T2SymbolInfos.ToList();
            //    //var all = a.Where(p => p != null && !string.IsNullOrWhiteSpace(p.Symbol) && (p.QuoteAsset == "USDT" || p.QuoteAsset == "BTC")).Select(s => s.Symbol);
            //    var symbolInfoEntities = new List<T2SymbolInfoEntity>();
            //    foreach (var p in binanceSymbols)
            //    {
            //        if (p != null && !string.IsNullOrWhiteSpace(p.Symbol) && (p.QuoteAsset == "USDT" || p.QuoteAsset == "BTC")
            //            && !dbSymbols2.Any(a => a.Symbol == p.Symbol))
            //        {
            //            symbolInfoEntities.Add(
            //                new T2SymbolInfoEntity
            //                {
            //                    BaseAsset = p.BaseAsset,
            //                    BaseAssetCommissionPrecision = p.BaseAssetCommissionPrecision,
            //                    BaseAssetPrecision = p.BaseAssetPrecision,
            //                    ExchangeType = p.ExchangeType,
            //                    QuoteAsset = p.QuoteAsset,
            //                    QuoteAssetCommissionPrecision = p.QuoteAssetCommissionPrecision,
            //                    QuoteAssetPrecision = p.QuoteAssetPrecision,
            //                    Symbol = p.Symbol
            //                }
            //                );
            //        }
            //    }
            //symbols = symbols.Where(p => !all.Contains(p.Symbol));
            //var symbolInfoEntities = symbols.Select(s => new Db.Entities.T2SymbolInfoEntity
            //{
            ...
            //    Symbol = s.Symbol
            //});
            //    db.T2SymbolInfos.AddRange(symbolInfoEntities);
            //    await db.SaveChangesAsync();
            //}
        ...
        private static async Task FixData()
        {
            using var db = new TradingToolsDbContext(optionsBuilder.Options);

            var inprogressGroups = await db.T2TradeGroups
                .Where(p => p.TradeGroupState == Db.Enums.TradeGroupState.InProgress && !p.IsDefault)
                .ToListAsync();
            var pendingTrades = await db.T2Trades.Where(p => p.TradeState == Db.Enums.TradeState.Pending).ToListAsync();

            foreach (var trade in pendingTrades)
            {
                var group = inprogressGroups.FirstOrDefault(f => f.SymbolInfoId == trade.T2SymbolInfoId);
                if(group == null)
                {

                }
                trade.T2TradeGroup = group;
            }

            await db.SaveChangesAsync();
        }

        private static async Task RenamedTradeGroups(TradingToolsDbContext db)
        {
            var tgs = await db.T2TradeGroups.Where(p => !p.IsDefault).Include(i => i.SymbolInfo).ToListAsync();
            var groups = tgs
                .OrderBy(x => x.Created)
            ...
            foreach (var item in groups)
            {
                ...
                {
                    var entity = item.ElementAt(i);
                    if (entity.SymbolInfo != null)
                    {
                        entity.Name = $"{item.Key.Year % 100}_{item.Key.Month:D2}-{i + 1}_{entity.SymbolInfo.Symbol}";
                        db.T2TradeGroups.Update(entity);
                    }
                }
            }
